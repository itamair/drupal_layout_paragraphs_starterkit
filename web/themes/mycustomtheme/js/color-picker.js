/**
 * @file
 * Provides UI/UX progressive enhancements on MyCustomTheme's theme settings by
...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.mycustomtheme.colorSchemes;
  ...
  function synchronizeInputs(changedInput, inputToSync) {
    ...
    changedInput.setAttribute('data-mycustomtheme-custom-color', changedInput.value);
    inputToSync.setAttribute('data-mycustomtheme-custom-color', changedInput.value);
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-mycustomtheme-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-mycustomtheme-custom-color');
        });
    }
  }
  ...
  /**
   * Initializes MyCustomTheme theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-mycustomtheme-custom-color',
      textInput.getAttribute('data-mycustomtheme-custom-color'),
    );
  ...
  /**
   * MyCustomTheme Color Picker behavior.
   *
   * @type {Drupal~behavior}
   * @prop {Drupal~behaviorAttach} attach
   *   Initializes color picker fields.
   */
  Drupal.behaviors.mycustomthemeColorPicker = {
    attach: () => {
      const colorSchemeSelect = once(
        'mycustomtheme-color-picker',
        '[data-drupal-selector="edit-color-scheme"]',
      );
      ...
      const colorTextInputs = once(
        'mycustomtheme-color-picker',
        '[data-drupal-selector="mycustomtheme-color-picker"] input[type="text"]',
      );
